...
<!-- 		</form> -->
		...
		<table class="table table-striped" style="color:white";>
			<thead>
				<tr>
					<th style="color:#fac564; text-align:center;">순위</th>
					<th style="color:#fac564; text-align:center;">검색번호</th>
					<th style="color:#fac564; text-align:center;">검색어</th>
					<th style="color:#fac564; text-align:center;">검색횟수</th>
					...
					<c:otherwise>
						<c:forEach var="dto" items="${list }">
							...
							<c:set var="count" value="${count+1}"/>
								...
								<td style="text-align:center;">${dto.fno}</td>
								<td style="text-align:center;">
								<a href="javascript:read('${dto.fno }')">${dto.fstr}</a></td>
								<td style="text-align:center;">${dto.fcnt}</td>
							<tr>
						</c:forEach>
					</c:otherwise>
				</c:choose>
...
<!-- Json ☞ csv convert		https://konklone.io/json/ -->
		...
		<button id="button1"
		class="btn btn-primary">DB 저장(CSV)</button>
		<button onclick="DelAll()"
		class="btn btn-primary">DB 초기화</button>
		<button onclick="Repair()"
		class="btn btn-primary">사진오류 수정</button>
		<button onclick="Chart()"
		class="btn btn-primary">차트 보기</button>
		</c:if>
		<div>
<%-- 			${paging} --%>
		</div>
