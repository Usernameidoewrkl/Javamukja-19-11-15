...
public class Utility {
		public static int tcount(int tno, TalkReplyInter tinter){
	        return tinter.tcount(tno);
	   }
	
		public static int rcount(int rno, RecipeReplyInter rinter){
	        return rinter.rcount(rno);
	   }
	
	public static int gradecount(int gno,GradeReplyInter grinter){
		  return grinter.gradecount(gno);
		}
	...
	public static String rpaging(int total, int nowPage, int recordPerPage, String col, String word, String url,
		...
		str.append("<span class='pagination2'> ");
		int _nowPage = (nowGrp - 1) * pagePerBlock; // 10개 이전 페이지로 이동

		if (nowGrp >= 2) {
			str.append("<span><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + nowPage + "&rno="
					+ rno + "&nPage=" + _nowPage + "'>이전</A></span>");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nPage == i) {
				str.append("<span class='active'><a href=#>" + i + "</a></span>");
			} else {
				str.append("<span><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + nowPage + "&rno="
						+ rno + "&nPage=" + i + "'>" + i + "</A></span>");
			}
		}
	...
	public static String paging(int totalRecord, int nowPage, int recordPerPage, String col, String word, String url) {
		...
		if (nowGrp >= 2) {
			str.append(
					"<span><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + _nowPage + "'>이전</A></span>&nbsp;&nbsp;");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nowPage == i) {
				...
				str.append("<span><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + i + "'>" + i
						+ "</A></span>&nbsp;&nbsp;");
			}
		}
		...
		if (nowGrp < totalGrp) {
			str.append(
					"<span><A href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + _nowPage + "'>다음</A></span>&nbsp;&nbsp;");
		}
		str.append("</span>");
		str.append("</div>");
